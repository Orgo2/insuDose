...
void TMP102_Deinit()
{
	...
	HAL_Delay(100);
	// disable (set to analog no-pull) for lowest leakage when sensor off
	GPIO_InitTypeDef GPIO_InitStruct = {0};
...
HAL_StatusTypeDef TMP102_SetAlarm(I2C_HandleTypeDef *hi2c, float t_low_c, float t_high_c, uint8_t fault_count)
{
    ...
    // Basic validation: t_low must be strictly less than t_high
    if (t_low_c >= t_high_c) {
        return HAL_ERROR;
    }

    // Clamp temperatures to TMP102 valid range (typical datasheet: -55 to +128 °C)
    if (t_low_c < -55.0f) t_low_c = -55.0f;
    if (t_high_c > 128.0f) t_high_c = 128.0f;
