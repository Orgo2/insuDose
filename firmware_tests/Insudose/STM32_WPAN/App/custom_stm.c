...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomSvcHdle;                  /**< insudos service handle */
  uint16_t  CustomIdHdle;                   /**< Insudos characteristic handle */
...
#define COPY_INSUDOS_SVC_UUID(uuid_struct) COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_INSUDOS_ID_UUID(uuid_struct)  COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for insudos +
   *                                2 for Insudos +
   *                              = 3
   *
  ...
  COPY_INSUDOS_SVC_UUID(uuid.Char_UUID_128);
  ...
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: id \n\r");
  }

  /**
   *  Insudos
   */
  COPY_INSUDOS_ID_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomSvcHdle,
                          UUID_TYPE_128, &uuid,
                          SizeId,
                          CHAR_PROP_NONE,
  ...
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : ID \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_ID:
      ret = aci_gatt_update_char_value(CustomContext.CustomSvcHdle,
                                       CustomContext.CustomIdHdle,
                                       0, /* charValOffset */
                                       SizeId, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value ID command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value ID command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_ID:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomSvcHdle, CustomContext.CustomIdHdle, SizeId, pPayload);
